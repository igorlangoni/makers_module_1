...
def first_three_elements(lista):
    return lista[0:3]
...
def we_joined_the_beatles(nmembers):
    beatles = ['john', 'paul', 'george', 'ringo']
    for m in nmembers:
        beatles.append(m)
    return beatles
...
def remove_nones_from_list(lista):
    nlista = []
    for item in lista:
        if item != None:
            nlista.append(item)
...
def touch_in(station, time):
    touch_in = {"entrypoint": station, "time": time}
    return touch_in